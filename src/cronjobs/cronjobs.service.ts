...
export class CronjobsService {
    ...
    async checkOnlineDevice(){
        ...
        list_device.map(async (device)=> {
            ...
            try{
                // if(log_devices.length > 0){
                //     console.log("log_devices[0]",log_devices[0])
    ...
    // @Cron(CronExpression.EVERY_MINUTE)
    // async rcfLogUpdate(){
    //     const TIME_OFFLINE_S = 600;
    //     let listRcf = await this.rcfSiteService.getAllRCF()
    //     listRcf.map(async (rcf)=>{
    //         try{

    //             let {powerats_device,powermain_device,temp_device} = rcf;
    ...
    //             // if(Site != null){
    //             //     (await this.lineservice.sendLineNotify({
    //             //         message:"Site : "+Site.site + " - Desc : "+Site.description,
    //             //     })).subscribe(response => console.log(response));
